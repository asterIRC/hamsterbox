...
static void m_remove(struct Client *, struct Client *, int, char *[]);
...
struct Message remove_msgtab = {
	"REMOVE", 0, 0, 3, 0, MFLG_SLOW, 0,
	{m_unregistered, m_remove, m_remove, m_ignore, m_remove, m_ignore}
};
 ...
 *  parv[0] = sender prefix
 *  parv[1] = channel
 *  parv[2] = client to force to part
 *  parv[3] = force part comment
 */
static void
m_remove(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	struct Client *who;
	struct Channel *chptr;
	int chasing = 0;
	...
	if((ms_target = find_channel_link(who, chptr)) != NULL)
	{
		...
		if(MyClient(source_p))
		{
			...
			if(IsNetAdmin(who) && source_p != who)
			{
				static char buf[IRCD_BUFSIZE];
				sendto_one(who,
					   ":%s NOTICE %s :*** %s tried to force part you from %s and failed",
					   ID_or_name(&me, who->from), ID_or_name(who, who->from),
					   source_p->name, name);
				ircsprintf(buf, "%s is a %s and cannot be kicked from %s",
					   who->name, "network administrator", name);
		 ...
		 */
		if(IsServer(source_p))
			sendto_channel_local(ALL_MEMBERS, NO, chptr, ":%s!%s@%s PART %s :Removed by server %s?!?!?! Reason: %s",
					     who->name, who->username, who->host, name, source_p->name, comment);
		else
			sendto_channel_local(ALL_MEMBERS, NO, chptr, ":%s!%s@%s PART %s :Requested by %s!%s@%s: %s",
					     who->name, who->username, who->host, name, source_p->name, source_p->username, source_p->host, comment);

		sendto_server(client_p, NULL, chptr, CAP_TS6, NOCAPS, NOFLAGS,
			      ":%s REMOVE %s %s :%s", ID(source_p), chptr->chname, ID(who), comment);
		sendto_server(client_p, NULL, chptr, CAP_TS6, NOCAPS, NOFLAGS,
			      ":%s KICK %s %s :%s", ID(source_p), chptr->chname, ID(who), comment);
		sendto_server(client_p, NULL, chptr, NOCAPS, CAP_TS6, NOFLAGS,
			      ":%s KICK %s %s :%s", source_p->name, chptr->chname,
